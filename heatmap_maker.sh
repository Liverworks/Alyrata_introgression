gene="AL2G25920"
outgr="SRR2040802,SRR2040803,SRR2040778,SRR2040779,SRR2040775,SRR2040776,SRR2040777"
gr_names=(Eur_dipl
Eur_tetr
CES_dipl
PUWS_dipl
CES_tetr
PUWS_tetr)
vcf_path="/netscratch/dep_mercier/grp_novikova/A.Lyrata/combine_vcf_lyrata2022/alyrata_plus_irku_scaff_2.bisnp.rh.vcf.gz"

#Group of samples on each line
...
# Get gene intervals
intervals=$(grep ${gene} /netscratch/dep_mercier/grp_novikova/A.Lyrata/S_locus/NT1_assembly/final_NT1/NT1_220222_fin2.gff3 | grep CDS | cut -f 1,4,5 | awk '{print $1":"$2"-"$3}' | paste -sd ',')
...
bcftools view --threads 20 -O z -s ${full_list},${outgr} -r ${intervals} --max-alleles 2 --force-samples --min-alleles 2 --exclude 'COUNT(GT="mis")>10' --types snps ${vcf_path} > ${gene}.snps.vcf.gz
...
bcftools view -H work.vcf | cut -f 4,5,17- | sed 's/:[0123456789,\._ATGC\|]*//g' | sed 's/\t/,/g' |  awk -F, '{am=0;tm=0;gm=0;cm=0;as=0;ts=0;gs=0;cs=0;af=0;tf=0;gf=0;cf=0; if($1 ~ /[aA]/)  am=gsub(/0/,"");if($1 ~ /[tT]/) tm=gsub(/0/,"");if($1 ~ /[gG]/) gm=gsub(/0/,"");if($1 ~ /[cC]/) cm=gsub(/0/,""); if($2 ~ /[aA]/)  am=gsub(/1/,"");if($2 ~ /[tT]/) tm=gsub(/1/,"");if($2 ~ /[gG]/) gm=gsub(/1/,"");if($2 ~ /[cC]/) cm=gsub(/1/,""); printf "%d,%d,%d,%d\n", am,cm,gm,tm }' > sfs_snps
